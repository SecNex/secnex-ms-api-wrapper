...
func (t *TeamsAPIBuilder) ListTeamsByName(filter string) string {
	log.Println("Filter: ", filter)
	// GET https://graph.microsoft.com/v1.0/teams?$filter=displayName eq 'A Contoso Team'&$select=id,description
	return t.BaseURL + "/v1.0" + TEAMS_LIST + "?$filter=displayName eq '" + filter + "'&$select=id,displayName,description"
}

func (t *TeamsAPIBuilder) ListTeamsWithLimit(limit int) string {
	return t.BaseURL + "/v1.0" + TEAMS_LIST + "?$select=id,displayName,description&$top=" + fmt.Sprintf("%d", limit)
}
